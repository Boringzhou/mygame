...
bool CTableFrame::StartGame()
{
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			if (pIServerUserItem!=NULL)
			{
				...
				else if(i == m_wFangzhu)
				{
					/*if (m_bKouFangka)
					{
						DedultInsure(pIServerUserItem, m_dwFangka );
					}
					else
					{*/
	...
	if (m_pITableFrameSink!=NULL&&bStart) 
	{
		...
		modifyRoom.dwUserID = GetTableUserItem(m_wFangzhu)->GetUserID();
		modifyRoom.roomInfo.dwRoomID = m_dwTablePassword;
		modifyRoom.roomInfo.cbSitPlayerCnt = GetSitUserCount();
		modifyRoom.roomInfo.cbRoomState = 1;//开始玩状态

		m_pIMainServiceFrame->SendData(MDM_CS_ROOM_SERVICE,SUB_CS_C_MODIFYROOM,&modifyRoom,sizeof(modifyRoom));
...
bool CTableFrame::AnyOneContinue()
{

	bool bAnyOneContinue = false;
	for (int i=0; i<m_wChairCount; i++)
	{
		IServerUserItem *pUser = GetTableUserItem(i);
		if(m_bContinue[i])
			bAnyOneContinue = true;
	}
	return bAnyOneContinue;
}
bool CTableFrame::ContinueGame(WORD wChairID)
{
	...
	{
		//如果没有定时器则返回
		if (!HasGameTimer(IDI_WAIT_CONTINUE))
		{
			return true;
		}
	}
	...
	if(m_bWaitForContinue)
	{
		...
		SCORE lZuanshi = GetZuanshi();

		if(lZuanshi>pIServerUserItem->GetUserScore())
		{
			SendRequestFailure(pIServerUserItem,TEXT("钻石不足，续费失败!"),REQUEST_FAILURE_NOZUANSHI);
	...
	//扣除房费
 	if(!bAnyOneContinue)
 	{
		if(GetZuanshi()>pIServerUserItem->GetUserScore())
		{
			SendRequestFailure(pIServerUserItem,TEXT("钻石不足，续费失败!"),REQUEST_FAILURE_NOZUANSHI);
	...
	for (int i=0; i<m_wChairCount; i++)
	{
		...
		if(!m_bContinue[i])
			bAllContinue = false;
		if(pUser->GetUserStatus()!=US_READY)
			bAllContinue = false;
	}

	if(bAllContinue && GetSitUserCount() == m_wChairCount)
	{
		StartGame();
		KillGameTimer(IDI_WAIT_CONTINUE);
	}
...
bool CTableFrame::ConcludeGame(BYTE cbGameStatus)
{
	...
	if (m_bGameStarted==false)
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			//启动定时
			/*if(IsGameStarted()==false && m_pGameServiceAttrib->wChairCount<MAX_CHAIR)
			{
...
//获取计时器剩余时间
DWORD CTableFrame::GetLeaveTime(DWORD dwTimerID)
{
	DWORD timeLeave = 0;
	ASSERT((dwTimerID>0)&&(dwTimerID<TIME_TABLE_MODULE_RANGE));
	if ((dwTimerID<=0)||(dwTimerID>=TIME_TABLE_MODULE_RANGE)) return false;
	...
	{
		timeLeave = m_pITimerEngine->GetLeaveTime(dwEngineTimerID+dwTimerID);
	}
	return timeLeave;
}
//是否存在计数器
bool CTableFrame::HasGameTimer(DWORD dwTimerID)
{
	//效验参数
	bool ret = false;
	ASSERT((dwTimerID>0)&&(dwTimerID<TIME_TABLE_MODULE_RANGE));
	...
	if (m_pITimerEngine!=NULL) 
	{
		ret = m_pITimerEngine->HasGameTimer(dwEngineTimerID+dwTimerID);
	}
	return ret;
}
...
bool CTableFrame::OnEventTimer(DWORD dwTimerID, WPARAM dwBindParameter)
{
	...
	case IDI_OFF_LINE:	//断线事件
		{
			...
			if(pIServerUserItem && pIServerUserItem->GetUserStatus() == US_OFFLINE)
				PerformStandUpAction(pIServerUserItem);
	...
	case IDI_WAIT_DISMISS:
		{
			...
			if (m_pITableFrameSink!=NULL) m_pITableFrameSink->OnEventGameConclude(INVALID_CHAIR,NULL,0xFE);
			return true;
		}
	...
	if(dwTimerID >= IDI_START_OVERTIME && dwTimerID <=IDI_START_OVERTIME_END)
	{
		...
		//	//用户起立
		//	//PerformStandUpAction(pIServerUserItem);
		//	pIServerUserItem->SetUserStatus(US_READY,pIServerUserItem->GetTableID(),wChair);
		//	KillGameTimer(IDI_START_OVERTIME+wChair);
		//}
	...
	case SUB_GF_GAME_OPTION:	//游戏配置
		{
			...
			//断线重连提示是否续费
			///////////////////////2017-05-05.断线重连提示消息修复////////////////////////
			if(!m_bContinue[wChairID] && m_bWaitForContinue) 
			{
				SendRequestFailure(pIServerUserItem,TEXT("该房间已经结束游戏,是否续费？"),REQUEST_FAILURE_END);
			}
			//////////////////////////////////////////////////////////////////////////////
			...
			for (int i=0; i<m_wChairCount; i++)
			{
				...
				UserStatus.dwUserScore = pUserItem->GetUserScore();
			...
			//	nDismissRoom.bAgree = m_bAgree[i];
			//	nDismissRoom.wUserID = pUserItem->GetUserID();
		
			//	m_pIMainServiceFrame->SendData(pIServerUserItem,MDM_GR_USER,SUB_GR_DISMISS_MSG,&nDismissRoom,sizeof(CMD_GR_DismissRoom));
			//}
	...
	case SUB_GF_HEADURL:		//头像地址
		{
			...
			CopyMemory(&(m_szHeadUrl[wChairID]),pHead->szHeadUrl,sizeof(m_szHeadUrl[wChairID]));

			//发送头像给其他人
			CMD_GF_S_HeadUrl nHead;
			nHead.wChairID = wChairID;
			CopyMemory(&(nHead.szHeadUrl),pHead->szHeadUrl,sizeof(nHead.szHeadUrl));
			...
			for (WORD i=0;i<m_wChairCount;i++)
			{
				...
				nHead.wChairID = i;
				CopyMemory(&(nHead.szHeadUrl),m_szHeadUrl[i],sizeof(nHead.szHeadUrl));
...
// 			UserExpression.dwSendUserID=pIServerUserItem->GetUserID();
			...
			//构造消息
 			...
 			UserExpression.wItemIndex=pUserExpression->wItemIndex;
 			UserExpression.dwSendUserID=pIServerUserItem->GetUserID();
			UserExpression.bGender=pIServerUserItem->GetGender();
...
void CTableFrame::SendDismissInfo(WORD wChairID)
{
	if (!HasGameTimer(IDI_WAIT_DISMISS))
	{
		return;
	}
	for (int i = 0;i != m_wChairCount;++i)
	{
		if (m_bResponseDismiss[i] == false)
		{
			continue;
		}
		auto userItem = GetTableUserItem(i);
		if (userItem)
		{
			CMD_GR_DismissRoom nDismissRoom;
			nDismissRoom.dwTimeLeave = GetLeaveTime(IDI_WAIT_DISMISS);
			nDismissRoom.bAgree = m_bAgree[i];
			nDismissRoom.wUserID = GetTableUserItem(i)->GetUserID();
			m_pIMainServiceFrame->SendData(GetTableUserItem(wChairID),MDM_GR_USER,SUB_GR_DISMISS_MSG,&nDismissRoom,sizeof(CMD_GR_DismissRoom));
		}
	}
}
...
{
	if (GetCreatorID() == 0 && m_pITableFrameSink)
	{
		m_pITableFrameSink->ResetbPlayer();
	}
	//效验参数
	ASSERT((pIServerUserItem!=NULL)&&(wChairID<m_wChairCount));
	ASSERT((pIServerUserItem->GetTableID()==INVALID_TABLE)&&(pIServerUserItem->GetChairID()==INVALID_CHAIR));
...
void CTableFrame::DedultInsure(IServerUserItem * pIServerUserItem,DWORD dwInsure)
{
	if (dwInsure < 0)
	{
		return ;
	...
	if(pIServerUserItem->GetUserInsure()<dwInsure)
	{
		return;
	}

	pIServerUserItem->WriteUserScore(0,0,0,0,SCORE_TYPE_LOSE,0,0,-dwInsure);
}
...
void CTableFrame::DedultScore(IServerUserItem * pIServerUserItem,SCORE lScore)
{
	...
	{
		CTraceService::TraceString(TEXT("金币不足以扣除房费！"),TraceLevel_Exception);
		return;
	}
...
bool CTableFrame::EfficacyStartGame(WORD wReadyChairID)
{
	...
	if(wReadyChairID!= INVALID_CHAIR)
	{
		if(!m_bContinue[wReadyChairID]) return false;
	}
...
void CTableFrame::StartReadyOutTime(BYTE wChairID)
{
	//SetGameTimer(IDI_START_OVERTIME+wChairID,TIME_OVERTIME,1,wChairID);
}
//////////////////////////////////////////////////////////////////////////////////
