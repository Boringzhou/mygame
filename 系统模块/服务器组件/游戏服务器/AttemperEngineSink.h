...
protected:
	...
	//创建房卡房间
	bool OnTCPNetworkSubCreateWordRoom(VOID * pData, WORD wDataSize, DWORD dwSocketID);	
	//加入房卡房间
	bool OnTCPNetworkSubJoinWordRoom(VOID * pData, WORD wDataSize, DWORD dwSocketID);
	//房间续费
	bool OnTCPNetworkSubContinue(VOID * pData, WORD wDataSize, DWORD dwSocketID);
	//解散房卡房间
	bool OnTCPNetworkSubDissmisRoom(VOID * pData, WORD wDataSize, DWORD dwSocketID);
...
protected:
	...
	//计算距离
	float GetDistance(double x1,double x2,double y1,double y2,float r);
};
