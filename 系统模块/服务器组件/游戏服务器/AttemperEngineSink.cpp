							...
							if (AndroidPassivity.wFreeUserCount>0)
							{
								...
								else
								{
									for (INT_PTR i=0;i<m_pGameServiceOption->wTableCount;i++)
									{
										...
										if (TableUserInfo.wTableUserCount==0) continue;
										if ((wUserSitCount>=TableUserInfo.wMinUserCount)&&(rand()%100>70)) continue;
						...
						else
						{
							...
							if (AndroidInitiative.wFreeUserCount>0)
							{
								for (INT_PTR i=0;i<m_pGameServiceOption->wTableCount;i++)
								{
									...
									//分配判断
									if((m_pGameServiceAttrib->wChairCount<MAX_CHAIR) && wUserSitCount>(TableUserInfo.wMinUserCount-1) && (rand()%100>70)) continue;
...
//发送数据
bool CAttemperEngineSink::SendData(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
...
bool CAttemperEngineSink::OnEventUserItemStatus(IServerUserItem * pIServerUserItem, WORD wOldTableID, WORD wOldChairID)
{
	...
	lstrcpyn(UserStatus.szSendNickName,pIServerUserItem->GetNickName(),sizeof(UserStatus.szSendNickName)/sizeof(TCHAR));
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	//特殊属性:DCY by 2017.3.11
	UserInfoPlus.m_dwJudging=pDBOLogonSuccess->dwJudging;
...
bool CAttemperEngineSink::OnDBExChangeResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	{
		SCORE lInsureScore=pIServerUserItem->GetUserInsure();
		SCORE lVarInsureScore=pExchangeResult->lCurrInsure-lInsureScore;
		pIServerUserItem->ModifyUserInsure(0,lVarInsureScore,0);
	}
...
bool CAttemperEngineSink::OnTCPNetworkMainUser(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_GR_USER_CREATE_ROOM:
		{
			return OnTCPNetworkSubCreateWordRoom(pData,wDataSize,dwSocketID);
		}
	case SUB_GR_USER_JOIN_ROOM:
		{
			return OnTCPNetworkSubJoinWordRoom(pData,wDataSize,dwSocketID);
		}
	case SUB_GR_USER_DISMISS_ROOM:
		{
			return OnTCPNetworkSubDissmisRoom(pData,wDataSize,dwSocketID);
		}
	case SUB_GR_USER_CONTINUE:
		{
			return OnTCPNetworkSubContinue(pData,wDataSize,dwSocketID);
		}
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubUserSitDown(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	//向协调服务登记房间信息
	CMD_CS_C_ModifyRoom modifyRoom;
	modifyRoom.dwUserID = pTableFrame->GetCreatorID();
	modifyRoom.roomInfo.dwRoomID = pTableFrame->GetPassword();
	modifyRoom.roomInfo.cbSitPlayerCnt = pTableFrame->GetSitUserCount();
	modifyRoom.roomInfo.cbRoomState = (modifyRoom.roomInfo.cbSitPlayerCnt==pTableFrame->GetChairCount() ? 1 : 0);

	m_pITCPSocketService->SendData(MDM_CS_ROOM_SERVICE,SUB_CS_C_MODIFYROOM,&modifyRoom,sizeof(modifyRoom));
...
bool CAttemperEngineSink::OnTCPNetworkSubUserStandUp(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if (wTableID!=INVALID_TABLE)
	{
		...
		{
			pTableFrame->KillGameTimer(24);

			//向协调服务登记房间信息
			CMD_CS_C_DeleteRoom deleteRoom;
			deleteRoom.dwUserID = pTableFrame->GetCreatorID();
			deleteRoom.dwRoomID   = pTableFrame->GetPassword();

			m_pITCPSocketService->SendData(MDM_CS_ROOM_SERVICE,SUB_CS_C_DELETEROOM,&deleteRoom,sizeof(deleteRoom));

			pTableFrame->m_pITableFrameSink->OnEventGameConclude(INVALID_CHAIR,NULL,0xFE);
		}
	}
...
bool CAttemperEngineSink::OnTCPNetwordSubSendTrumpet(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	lstrcpyn(SendTrumpet.szSendNickName,pIServerUserItem->GetNickName(),sizeof(SendTrumpet.szSendNickName));
...
//创建房卡房间
bool CAttemperEngineSink::OnTCPNetworkSubCreateWordRoom(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	printf("系统模块-客户端请求登陆--创建房卡房间---  验证 - [ 1 ]\n");
	//获取用户
	WORD wBindIndex=LOWORD(dwSocketID);
	IServerUserItem * pIServerUserItem=GetBindUserItem(wBindIndex);
	...
	ASSERT(wDataSize==sizeof(CMD_GR_Create_Room));
	if(wDataSize != sizeof(CMD_GR_Create_Room))
	{
		SendRequestFailure(pIServerUserItem,TEXT("网络数据错误！"),REQUEST_FAILURE_NORMAL);
		return true;
	}
	printf("系统模块-客户端请求登陆--创建房卡房间---  验证 - [ 2 ]\n");
	CMD_GR_Create_Room *pCreateRoom = (CMD_GR_Create_Room*)pData;

	if (pCreateRoom->cbGameCount != 10 && pCreateRoom->cbGameCount != 20 && pCreateRoom->cbGameCount != 30&& pCreateRoom->cbGameCount != 50)
	{
		return false;
	}

	if (pCreateRoom->cbPlayerCount != 2 && pCreateRoom->cbPlayerCount != 3 && pCreateRoom->cbPlayerCount != 4)
	{
		return false;
	}
	...
	//房卡计算
	SCORE nFangka = 1;
	if(pCreateRoom->cbGameCount == 10)
	{
		nFangka = 1;
	}
	else if(pCreateRoom->cbGameCount == 20)
	{
		nFangka = 2;
	}
	else if(pCreateRoom->cbGameCount == 30)
	{
		nFangka = 3;
	}
	//钻石计算
	...
	if(pCreateRoom->cbGameCount == 10)
	{
		nZuanshi=7*pCreateRoom->cbPlayerCount;
	}
	else if(pCreateRoom->cbGameCount == 20)
	{
		nZuanshi=14*pCreateRoom->cbPlayerCount;
	}
	else if(pCreateRoom->cbGameCount == 30)
	{
		nZuanshi=21*pCreateRoom->cbPlayerCount;
	}
	else if(pCreateRoom->cbGameCount == 50)
	{
		nZuanshi=35*pCreateRoom->cbPlayerCount;
	}
	
	if(pCreateRoom->cbPayType == 0)
	{
		if(pCreateRoom->cbPlayerCount == 0)
		{
			SendRequestFailure(pIServerUserItem,TEXT("游戏人数配置异常，请重新尝试!"),REQUEST_FAILURE_NORMAL);
			return true;
		}

		if(nZuanshi>pIServerUserItem->GetUserScore())
		{
			SendRequestFailure(pIServerUserItem,TEXT("钻石不足,无法创建房间！"),REQUEST_FAILURE_NORMAL);
			return true;
		}
	}
	else
	{
		...
		DWORD tmpZuan = (nZuanshi/pCreateRoom->cbPlayerCount);
		if(tmpZuan>pIServerUserItem->GetUserScore())
		{
			SendRequestFailure(pIServerUserItem,TEXT("钻石不足,无法创建房间！"),REQUEST_FAILURE_NORMAL);
			return true;
		}
	...
	printf("系统模块-客户端请求登陆--创建房卡房间---  验证 - [ 3 ]\n");


	bool bKouFangka = false;
	if (nFangka < pIServerUserItem->GetUserInsure())
	{
		/*bKouFangka = true;*/
	}

	printf("系统模块-客户端请求登陆--创建房卡房间---  验证 - [ 5 ]\n");
	/////////////////////////

	printf("创建房间 USER--%d TABLE--%d CHIAR--%d SOCKID--%d\n",pIServerUserItem->GetUserID(),pIServerUserItem->GetTableID(),pIServerUserItem->GetChairID(),dwSocketID);
	...
	{
		//SendData(pIServerUserItem,MDM_GR_USER,SUB_GR_JOIN_NULL,NULL,0);
	...
	srand(time(NULL));
	DWORD dwPassword = ((1+rand()%5)<<17) + ((rand()%512)<<8) + m_pGameServiceOption->wServerID;
	...
	if (wChairID!=INVALID_CHAIR)
	{
		while(true)
		{
			...
			for (WORD i=0;i<m_TableFrameArray.GetCount();i++)
			{
				...
				//状态判断
				if (/*pTableFrame->GetSitUserCount()>0 && */pTableFrame->GetPassword() == dwPassword)
				{
					bFind = true;
					break;
				}
			...
			if(bFind) 
				dwPassword = ((1 + rand() % 5) << 17) + ((rand() % 512) << 8) + m_pGameServiceOption->wServerID;
			else
				break;
		}
		...
		//	if (pTableFrame->GetSitUserCount()==0)
		//	{
		//		srand(m_pGameServiceOption->wServerID);
		//		dwPassword = ((i+1)<<10)|m_pGameServiceOption->wServerID+((rand()%875)<<10);
		//		break;
		//	}
		//}
		...
		pCurrTableFrame->SetCreatorID(pIServerUserItem->GetUserID());
		pCurrTableFrame->SetFangzhu(wChairID);
		pCurrTableFrame->SetContinue(wChairID);
		pCurrTableFrame->SetPassword(dwPassword);
		pCurrTableFrame->SetPayType(pCreateRoom->cbPayType);
		pCurrTableFrame->SetKing(pCreateRoom->bKing);
		pCurrTableFrame->SetZuanshi(nZuanshi);
		pCurrTableFrame->SetInsure(nFangka);
		pCurrTableFrame->SetKouFangka(bKouFangka);
		pCurrTableFrame->SetRefuseSameIP(pCreateRoom->bRefuseSameIP);
		pCurrTableFrame->SetDistanceIn300(pCreateRoom->bDistanceIn300);
		pCurrTableFrame->SetLongitude(wChairID,pCreateRoom->dLongitude);
		pCurrTableFrame->SetLatitude(wChairID,pCreateRoom->dLatitude);
	}
	else
	{
		SendRequestFailure(pIServerUserItem,TEXT("您已经在游戏中,不能重新创建房间!"),REQUEST_FAILURE_NORMAL);
		return true;
	}

	//发送房间消息
	CMD_GR_CreateSuccess nCreate;
	nCreate.dwRoomID = dwPassword;
	nCreate.wChairID = wChairID;
	SendData(pIServerUserItem,MDM_GR_USER,SUB_GR_CREATE_SUCCESS,&nCreate,sizeof(CMD_GR_CreateSuccess));

	//向协调服务登记房间信息
	CMD_CS_C_AddRoom addRoom;
	addRoom.dwUserID = pIServerUserItem->GetUserID();
	addRoom.roomInfo.dwCreatorID = pIServerUserItem->GetUserID();
	addRoom.roomInfo.dwRoomID = dwPassword;
	addRoom.roomInfo.wServerID = m_pGameServiceOption->wServerID;
	addRoom.roomInfo.cbJuShu = pCreateRoom->cbGameCount;
	addRoom.roomInfo.cbPlayer = pCreateRoom->cbPlayerCount;
	addRoom.roomInfo.cbSitPlayerCnt = 1;//刚创建的房间默认1
	addRoom.roomInfo.cbRoomState = 0;

	m_pITCPSocketService->SendData(MDM_CS_ROOM_SERVICE,SUB_CS_C_ADDROOM,&addRoom,sizeof(addRoom));
	 
	return true;
}

//加入房卡房间
bool CAttemperEngineSink::OnTCPNetworkSubJoinWordRoom(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//获取用户
	WORD wBindIndex=LOWORD(dwSocketID);
	IServerUserItem * pIServerUserItem=GetBindUserItem(wBindIndex);
	...
	CMD_GR_Join_Room *pJoin = (CMD_GR_Join_Room *)pData;

	//srand(pJoin->dwRoomNum&0x3FF);
	//int nTableID = ((pJoin->dwRoomNum-(rand()%875))>>10)-1;

	printf("加入房间 [%d] [%d] [%d]\n",pJoin->dwRoomNum,pIServerUserItem->GetTableID(),pIServerUserItem->GetChairID());

	WORD wChairID = pIServerUserItem->GetChairID();

	DWORD dwPassword = pJoin->dwRoomNum;
	...
	if (pIServerUserItem->GetChairID()==INVALID_CHAIR && pIServerUserItem->GetTableID()==INVALID_TABLE)
	{
		if(dwPassword == 0)
		{
			SendRequestFailure(pIServerUserItem,TEXT("房间号不存在或已解散,请重新尝试!"),REQUEST_FAILURE_NORMAL);
			return true;
		}
		...
		for (WORD i=0;i<m_TableFrameArray.GetCount();i++)
		{
			...
			//状态判断
			if (pTableFrame->GetPassword()==dwPassword)
			{
				pCurrTableFrame=pTableFrame;
				break;
			}
		...
		{
			if(pCurrTableFrame->GetZuanshi()>pIServerUserItem->GetUserScore())
			{
				SendRequestFailure(pIServerUserItem,TEXT("钻石不足,无法加入房间！"),REQUEST_FAILURE_NORMAL);
				return true;
			}

			//校验同IP
			if(pCurrTableFrame->GetRefuseSameIP())
			{
				DWORD dwUserIP=pIServerUserItem->GetClientAddr();
				for(int i=0; i<pCurrTableFrame->GetChairCount(); i++)
				{
					IServerUserItem * pTableUserItem = pCurrTableFrame->GetTableUserItem(i);
					if(pTableUserItem == NULL) continue;
					DWORD dwTableUserIP=pTableUserItem->GetClientAddr();
					if(dwUserIP == dwTableUserIP)
					{
						SendRequestFailure(pIServerUserItem,TEXT("该房间不允许相同IP的玩家一起游戏!"),REQUEST_FAILURE_NORMAL);
						return true;
					}
				}
			}
			//校验距离
			if(pCurrTableFrame->GetDistanceIn300())
			{
				double dLongitude = pJoin->dLongitude;
				double dLatitude = pJoin->dLatitude;
				...
				for(int i=0; i<pCurrTableFrame->GetChairCount(); i++)
				{
					...
					double dTableLongitude = pCurrTableFrame->GetLongitude(i);
					double dTableLatitude = pCurrTableFrame->GetLatitude(i);
					if((dLongitude<0.01 && dLongitude>-0.01)||(dLatitude<0.01 && dLatitude>-0.01))
					{
						SendRequestFailure(pIServerUserItem,TEXT("获取定位失败,房主拒绝您的加入!!"),REQUEST_FAILURE_NORMAL);
						return true;
					}
					if(GetDistance(dLongitude,dTableLongitude,dLatitude,dTableLatitude,6378.14f)<300)
					{
						SendRequestFailure(pIServerUserItem,TEXT("该房间不允许距离300米内的玩家一起游戏!!"),REQUEST_FAILURE_NORMAL);
						return true;
					}
				}
			}
		...
		if (wChairID!=INVALID_CHAIR)
		{
			if(pCurrTableFrame->PerformSitDownAction(wChairID,pIServerUserItem)==false)
			{
				SendRequestFailure(pIServerUserItem,TEXT("坐下失败!"),REQUEST_FAILURE_PASSWORD);
				return true;
			}	
		...
		//向协调服务登记房间信息
		CMD_CS_C_ModifyRoom modifyRoom;
		modifyRoom.dwUserID = pCurrTableFrame->GetCreatorID();
		modifyRoom.roomInfo.dwRoomID = pCurrTableFrame->GetPassword();
		modifyRoom.roomInfo.cbSitPlayerCnt = pCurrTableFrame->GetSitUserCount();
		modifyRoom.roomInfo.cbRoomState = 0;
	...
	else
	{
		...
		if(m_TableFrameArray[pIServerUserItem->GetTableID()]->GetPassword() == 0)
		{
			SendRequestFailure(pIServerUserItem,TEXT("该房间已经结束游戏!"),REQUEST_FAILURE_NORMAL);

			m_TableFrameArray[pIServerUserItem->GetTableID()]->PerformStandUpAction(pIServerUserItem);
			return true;
		}
		///////////////////////2017-05-05.游戏中断线重连金币不足修复////////////////////////
		if(m_TableFrameArray[pIServerUserItem->GetTableID()]->GetGameStatus() == GAME_STATUS_FREE && m_TableFrameArray[pIServerUserItem->GetTableID()]->GetZuanshi()>pIServerUserItem->GetUserScore())
		{
			printf("断线重连 -- 金币不足 %d %d\n",m_TableFrameArray[pIServerUserItem->GetTableID()]->GetZuanshi(),pIServerUserItem->GetUserScore());
			SendRequestFailure(pIServerUserItem,TEXT("钻石不足,无法加入房间！"),REQUEST_FAILURE_NORMAL);
		...
		{
			printf("重新加入游戏\n");
			dwPassword = m_TableFrameArray[pIServerUserItem->GetTableID()]->GetPassword();
		}
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubContinue(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	printf("系统模块---(接受到客户端这边传来的数据消息)  验证-1\n");
	//获取用户
	WORD wBindIndex=LOWORD(dwSocketID);
	IServerUserItem * pIServerUserItem=GetBindUserItem(wBindIndex);
	if (pIServerUserItem==NULL) return true;
	printf("系统模块---(接受到客户端这边传来的数据消息)  验证-2\n");
	...
	printf("系统模块---(接受到客户端这边传来的数据消息)  验证-3\n");

	CTableFrame * pTableFrame=m_TableFrameArray[pIServerUserItem->GetTableID()];
	pTableFrame->ContinueGame(pIServerUserItem->GetChairID());

	return true;
}

//解散房间
bool CAttemperEngineSink::OnTCPNetworkSubDissmisRoom(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	//获取用户
	WORD wBindIndex=LOWORD(dwSocketID);
	IServerUserItem * pIServerUserItem=GetBindUserItem(wBindIndex);
	...
	WORD wTableID = pIServerUserItem->GetTableID();
	if(wTableID==INVALID_TABLE) return false;
	CTableFrame * pCurrTableFrame=m_TableFrameArray[wTableID];
	if(pCurrTableFrame == NULL) return false;

	DWORD dwRoomID = pCurrTableFrame->GetPassword();
	DWORD dwCreatorID  = pCurrTableFrame->GetCreatorID();

	CMD_GR_DismissRoom *pAgree = (CMD_GR_DismissRoom*)pData;
	if(dwCreatorID!=0 && m_TableFrameArray[wTableID]->OnEventDismissRoom(pIServerUserItem->GetChairID(), pAgree->bAgree))
	{
		m_TableFrameArray[wTableID]->SetCreatorID(0);
		for(int i=0; i<pCurrTableFrame->GetChairCount(); i++)
		{
			IServerUserItem * pTableUserItem = pCurrTableFrame->GetTableUserItem(i);
			if(pTableUserItem == NULL) continue;

			PerformUnlockScore(pTableUserItem->GetUserID(),pTableUserItem->GetInoutIndex(),LER_SYSTEM);
		}

		//向协调服务登记房间信息
		CMD_CS_C_DeleteRoom deleteRoom;
		deleteRoom.dwUserID = dwCreatorID;
		deleteRoom.dwRoomID   = dwRoomID;
...
bool CAttemperEngineSink::OnTCPNetworkSubSaveScoreRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	{
		//发送数据
		SendInsureFailure(pIServerUserItem,TEXT("此类型房间禁止存款，存入操作失败！"),0L,pSaveScoreRequest->cbActivityGame);
		return true;
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubTakeScoreRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	{
		//发送数据
		SendInsureFailure(pIServerUserItem,TEXT("此类型房间禁止取款，取出操作失败！"),0L,pTakeScoreRequest->cbActivityGame);
		return true;
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubTransferScoreRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	{
		//发送数据
		SendInsureFailure(pIServerUserItem,TEXT("此类型房间禁止转账，转账操作失败！"),0L,pTransferScoreRequest->cbActivityGame);
		return true;
	}
...
VOID CAttemperEngineSink::OnEventUserLogon(IServerUserItem * pIServerUserItem, bool bAlreadyOnLine)
{
	...
	if (pIServerUserItem->IsMobileUser()==false)
	{
		...
		SendData(pBindParameter->dwSocketID,MDM_GR_STATUS,SUB_GR_TABLE_INFO,&TableInfo,sizeof(TableInfo));
...
bool CAttemperEngineSink::SwitchUserItemConnect(IServerUserItem * pIServerUserItem, TCHAR szMachineID[LEN_MACHINE_ID], WORD wTargetIndex,BYTE cbDeviceType,WORD wBehaviorFlags,WORD wPageTableCount)
{
	...
	//状态切换
 	...
 	//{
		...
		{
			if (m_TableFrameArray[wTableID]->IsGameStarted())
			{
				pIServerUserItem->SetUserStatus(US_PLAYING,wTableID,wChairID);
			}
			else
...
bool CAttemperEngineSink::SendRequestFailure(IServerUserItem * pIServerUserItem, LPCTSTR pszDescribe, LONG lErrorCode)
{
	...
	printf("系统模块-客户端请求登陆--创建房卡房间---  验证 - [ 请求失败 ]\n");
...
bool CAttemperEngineSink::SendUserInfoPacket(IServerUserItem * pIServerUserItem, DWORD dwSocketID,bool bSendAndroidFalg)
{
	...
	pUserInfoHead->dwAddress = pIServerUserItem->GetClientAddr();
...
float CAttemperEngineSink::GetDistance(double x1,double x2,double y1,double y2,float r)
{
	float mod;
	mod=r*acos(sin(x1)*sin(x2)+cos(x1)*cos(x2)*cos(y1-y2));
	return mod*1000;
}
//更新控制
