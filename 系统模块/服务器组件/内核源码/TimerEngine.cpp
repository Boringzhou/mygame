...
//是否有计时器
bool CTimerEngine::HasGameTimer(DWORD dwTimerID)
{
	bool ret = false;
	//锁定资源
...
DWORD CTimerEngine::GetLeaveTime(DWORD dwTimerID)
{
	DWORD timeLeave = 0;
	//锁定资源
	CWHDataLocker lock(m_CriticalSection);
	...
	for (INT_PTR i = 0; i < m_TimerItemActive.GetCount(); i++)
	{
		...
		if (pTimerItem->wTimerID == dwTimerID)
		{
			timeLeave = pTimerItem->dwTimeLeave;
			break;
		}
	}
	return timeLeave;
...
bool  CTimerEngine::SetTimer(DWORD dwTimerID, DWORD dwElapse, DWORD dwRepeat, WPARAM dwBindParameter)
{
	...
	if (pTimerItem->dwRepeatTimes == 1)
		pTimerItem->dwTimeLeave = __max((int)TIMER_SPACE, (int)pTimerItem->dwElapse - (int)TIMER_SPACE * 20);
	else
