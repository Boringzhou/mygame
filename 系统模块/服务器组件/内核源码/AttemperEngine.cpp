	...
	switch (wIdentifier)
	{
		case EVENT_TIMER:			//定时器事件
		{
			...
			catch (...)
			{
				...
				_sntprintf(szInfo,sizeof(szInfo),TEXT("内核引擎定时器事件有错误！"));
		...
		case EVENT_DATABASE:		//数据库事件
		{
			...
			{
				static TCHAR szInfo[250] = {0};
				_sntprintf(szInfo,sizeof(szInfo),TEXT("内核引擎数据库事件有错误！"));
				//printf("内核引擎数据库事件有错误！\n");
			}
			return;
		}
		...
		case EVENT_TCP_NETWORK_ACCEPT:	//网络应答事件
		{
			...
			{
				static TCHAR szInfo[250] = {0};
				_sntprintf(szInfo,sizeof(szInfo),TEXT("内核引擎网络应答事件有错误！"));
				//printf("内核引擎网络应答事件有错误！\n");
			}
			if (bSuccess == false)
			{
				//printf("关闭连接 3\n");
		...
		case EVENT_TCP_NETWORK_READ:	//网络读取事件
		{
			...
			try
			{
				printf("读取事件 %d %d\n",pSocketReadEvent->Command, pSocketReadEvent + 1);
			...
			{
				static TCHAR szInfo[250] = {0};
				_sntprintf(szInfo,sizeof(szInfo),TEXT("内核引擎网络读取事件有错误！"));
				printf("内核引擎网络读取事件有错误！\n");
			}
			if (bSuccess == false)
			{
				printf("读取事件错误 %d %d\n",pSocketReadEvent->Command.wMainCmdID,pSocketReadEvent->Command.wSubCmdID);
		...
		{
			printf("内核 EVENT_TCP_NETWORK_SHUT\n");
			//效验大小
			ASSERT(wDataSize == sizeof(NTY_TCPNetworkShutEvent));
			if (wDataSize != sizeof(NTY_TCPNetworkShutEvent)) return;

			//处理消息
			NTY_TCPNetworkShutEvent * pSocketCloseEvent = (NTY_TCPNetworkShutEvent *)pBuffer;

			bool bSuccess = false;
			try
			{
				bSuccess = m_pIAttemperEngineSink->OnEventTCPNetworkShut(pSocketCloseEvent->dwClientAddr, pSocketCloseEvent->dwActiveTime, pSocketCloseEvent->dwSocketID);
			}
			catch (...)
			{
				static TCHAR szInfo[250] = {0};
				_sntprintf(szInfo,sizeof(szInfo),TEXT("内核引擎关闭读取事件有错误！"));
				printf("内核引擎关闭读取事件有错误！\n");
			}
			if (bSuccess == false)
			{
				printf("关闭连接 4\n");
		...
		case EVENT_TCP_SOCKET_SHUT:	//网络关闭事件
		{
			...
			//处理消息
			NTY_TCPSocketShutEvent * pSocketCloseEvent = (NTY_TCPSocketShutEvent *)pBuffer;
			...
			{
				static TCHAR szInfo[250] = {0};
				_sntprintf(szInfo,sizeof(szInfo),TEXT("内核引擎网络关闭事件有错误！"));
				OutputDebugString(szInfo);
				//printf("内核引擎网络关闭事件有错误！\n");
			}
			if (bSuccess == false)
			{
				//printf("关闭连接 6\n");
		...
		case EVENT_TCP_SOCKET_LINK:
		{
			...
			//处理消息
			NTY_TCPSocketLinkEvent * pSocketCloseEvent = (NTY_TCPSocketLinkEvent *)pBuffer;
