...
VOID CDataBaseAide::GetParameter(LPCTSTR pszItem, LPSTR pszString, UINT uSize)
{
	if (m_pIDataBase != NULL)
	{
		...
		((CDataBase*)m_pIDataBase)->GetParameter(pszItem, DBVarValue);
		CopyMemory(pszString,(char*)_bstr_t( DBVarValue ),uSize);
	}
}
...
VOID CDataBaseAide::AddParameter(LPCTSTR pszItem, LONGLONG lValue, ParameterDirectionEnum ParameterDirection)
{
	if (m_pIDataBase != NULL)
	{
		((CDataBase*)m_pIDataBase)->AddParameter(pszItem, adBigInt, adParamInput, sizeof(LONGLONG), _variant_t((LONGLONG)lValue));
	}
}
...
//插入参数
VOID CDataBaseAide::AddParameterOutput(LPCTSTR pszItem, LPCTSTR pszString, UINT uSize, ParameterDirectionEnum ParameterDirection)
{
	if (m_pIDataBase != NULL)
	{
		((CDataBase*)m_pIDataBase)->AddParameter(pszItem, adVarChar, ParameterDirection, uSize, _variant_t(""));
	}
}
