...
#define TIME_BREAK_CONNECT			4500L								//中断时间
...
bool CServerSocketItem::OnRecvCompleted(COverLappedRecv * pOverLappedRecv, DWORD dwThancferred)
{
	...
	try
	{
		while (m_wRecvSize >= sizeof(TCP_Head))
		{
			...
			if (pHead->TCPInfo.cbDataKind != SOCKET_VER) throw TEXT("数据包版本错误");
...
// 	}
	...
	try
	{
		...
		if(m_wListenPort == 0)
		{
			int sin_size = sizeof(SocketAddr);
			getsockname(m_hServerSocket, (struct sockaddr*)&SocketAddr, &sin_size);
			m_wListenPort=ntohs(SocketAddr.sin_port);
		}
