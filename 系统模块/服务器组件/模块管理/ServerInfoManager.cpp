...
//加载奖励
WORD CServerInfoManager::LoadGameMatchReward(DWORD dwMatchID,DWORD dwMatchNO, tagGameMatchOption * pGameMatchResult)
{
	//变量定义
	CDataBaseAide PlatformDBAide;
	CDataBaseHelper PlatformDBModule;
	...
	try
	{
		...
		{
			pGameMatchResult->wRewardCount=(WORD)PlatformDBModule->GetRecordCount();

			//读取列表
			WORD wIndex=0;
			while (PlatformDBModule->IsRecordsetEnd()==false)
			{
				pGameMatchResult->MatchRewardInfo[wIndex].wRankID=PlatformDBAide.GetValue_WORD(TEXT("MatchRank"));
				pGameMatchResult->MatchRewardInfo[wIndex].lRewardGold=PlatformDBAide.GetValue_LONGLONG(TEXT("RewardGold"));
				pGameMatchResult->MatchRewardInfo[wIndex].dwRewardIngot=PlatformDBAide.GetValue_DWORD(TEXT("RewardMedal"));
				pGameMatchResult->MatchRewardInfo[wIndex].dwRewardExperience=PlatformDBAide.GetValue_DWORD(TEXT("RewardExperience"));
			...
			ASSERT(pGameMatchResult->wRewardCount==wIndex);

			return pGameMatchResult->wRewardCount;
		}		
...
bool CServerInfoManager::ReadGameMatchOption(CDataBaseAide & PlatformDBAide, tagGameMatchOption & GameMatchResult)
{
	...
	//索引变量
	GameMatchResult.dwMatchID=PlatformDBAide.GetValue_DWORD(TEXT("MatchID"));
	GameMatchResult.dwMatchNO=PlatformDBAide.GetValue_DWORD(TEXT("MatchNo"));

	//费用信息
	GameMatchResult.cbMatchFeeType= PlatformDBAide.GetValue_BYTE(TEXT("MatchFeeType"));
	GameMatchResult.lMatchFee= PlatformDBAide.GetValue_LONGLONG(TEXT("MatchFee"));

	//比赛信息
	GameMatchResult.cbMemberOrder = PlatformDBAide.GetValue_BYTE(TEXT("MemberOrder"));
	GameMatchResult.cbMatchType = PlatformDBAide.GetValue_BYTE(TEXT("MatchType"));
