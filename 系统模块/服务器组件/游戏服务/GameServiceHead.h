...
#include "..\..\服务器组件\内核源码\KernelEngineHead.h"
...
#define START_MODE_TIME_FISHCONTROL	0x12							    //捕鱼时间控制 2017.1.22 Add 供捕鱼使用
...
#define SUB_S_RECORD				(0x0001)									//录像
#define SUB_S_RECORD_HEAD			(0x0002|SUB_S_RECORD)						//录像信息
...
enum enOption
{
	...
	enOption_Score,
	enOption_Cell,
	enOption_PlayerCount,
	enOption_PlayType,
	enOption_IsIp,
	enOption_IsAdd,
	enOption_PayType,
	enOption_custom,
	enOption_Five_Min=19,
	enOption_Spec
};
...
//游戏配置
struct tagFangkaGameOption
{
	BYTE							cbGameOptionType[30];				//配置属性
};
...
struct tagUserInfoPlus
{
	...
	//特殊属性
	DWORD							m_dwJudging;						//玩家阴德DCY:by 2017.3.11
...
public:
	//用户阴德:DCY by 2017.3.11
	virtual DWORD GetUserJudging()=NULL;
...
public:
	...
	//写入积分
	virtual bool WriteUserScore(SCORE lScore, SCORE lGrade, SCORE lRevenue, SCORE lIngot, BYTE cbScoreType, DWORD dwPlayTimeCount,DWORD dwWinExperience,DWORD dwInsure = 0)=NULL;
	//领取奖励
...
public:
	...
	virtual tagGameServiceOption * GetGameServiceOption()=NULL;
	//房卡配置
	virtual tagFangkaGameOption * GetFangkaGameOption()=NULL;
...
public:
	...
	//设置状态
	virtual void SetFangkaStart(bool bStart)=NULL;
	virtual bool AnyOneContinue()=NULL;
	//控制接口
public:
	//开始游戏
...
public:
	...
	//是否有计时器
	virtual bool HasGameTimer(DWORD dwTimerID)=NULL;
	//获取计时器剩余时间
	virtual DWORD GetLeaveTime(DWORD dwTimerID)=NULL;
	//网络接口
public:
	//发送数据
...
public:
	...
	virtual void SendDismissInfo(WORD wChairID)=NULL;
	//比赛接口
public:	
	//获取接口
	...
	virtual void StartReadyOutTime(BYTE wChairID) = NULL;
...
public:
	...
	virtual void ResetbPlayer()=NULL;
	//游戏事件
public:
	//游戏开始
