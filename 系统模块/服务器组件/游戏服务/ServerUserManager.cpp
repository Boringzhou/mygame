...
//报名状态
VOID CServerUserItem::SetSignUpStatus(BYTE cbSignUpStatus)
{
	BYTE cbOldMatchStatus=m_cbSignUpStatus;

	m_cbSignUpStatus=cbSignUpStatus;

	//发送状态
	ASSERT(m_pIMatchUserItemSink!=NULL);
	if (m_pIMatchUserItemSink!=NULL) m_pIMatchUserItemSink->OnEventMatchUserStatus(this,cbOldMatchStatus,cbSignUpStatus);
}

//写入积分
bool CServerUserItem::WriteUserScore(SCORE lScore, SCORE lGrade, SCORE lRevenue, SCORE lIngot, BYTE cbScoreType, DWORD dwPlayTimeCount,DWORD dwWinExperience,DWORD dwInsure)
{
	printf("WriteUserScore 写入积分\n");
	//效验状态
...
{
	printf("ModifyUserProperty 修改信息\n");
	//效验状态
	ASSERT(m_UserInfo.dwUserID!=0L);
	if (m_UserInfo.dwUserID==0L) return false;
...
bool CServerUserManager::InsertUserItem(IServerUserItem * * pIServerUserResult, tagUserInfo & UserInfo,tagUserInfoPlus & UserInfoPlus)
{
	...
	pServerUserItem->m_dwJudging=UserInfoPlus.m_dwJudging;
