...
{try{
	...
	case DBO_GP_SYSTEM_QQ:				// 查询系统QQ:DCY by 2017.3.9
		{
			return OnDBSystemQQ(dwContextID,pData,wDataSize);
		}
	case DBO_GP_SYSTEM_INFO:			//查询系统公告:DCY by 2017.3.9
		{
	...
	case DBO_GP_SYSTEM_SHOP:			//查询游戏商城:DCY by 2017.3.11
		{
			return OnDBGameShop(dwContextID,pData,wDataSize);
		}
	case DBO_GP_EXCHANGE_SHOP:			//兑换游戏商品:DCY by 2017.3.11
		{
...
bool CAttemperEngineSink::OnTCPNetworkMainPCUserService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_GP_QUERY_SYSTEMQQ:			//系统QQ:DCY by 2017.3.9
		{
			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_QUERY_SYSTEM_QQ,dwSocketID,0,0);
			return true;
		}
	case SUB_GP_QUERY_SYSTEMINFO:		//系统消息:DCY by 2017.3.9
		{
			//投递请求
	...
	case SUB_GP_QUERY_SHOP:				//系统商城:DCY by 2017.3.11
		{
			//投递请求
			m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_QUERY_SYSTEM_SHOP,dwSocketID,0,0);
			return true;
		}
	case SUB_GP_EXCHANGE_SHOP:			//兑换商品:DCY by 2017.3.11
		{
			//效验参数
	...
	for (INT i=0; i<m_VerifyCodeArray.GetCount();i++)
	{
		...
		if (pVerifyCode->dwClientAddr==dwClientAddr && lstrcmp(pVerifyCode->szMachineID,pRegisterAccounts->szMachineID)==0)
		{
			if (CheckVerifyCode(pRegisterAccounts->szVerifyCode,pVerifyCode->szValidateCode)==false)
			{
				...
				lstrcpyn(LogonFailure.szDescribeString,TEXT("校验码校验失败！"),CountArray(LogonFailure.szDescribeString));
...
// 	_sntprintf(exInfo.strName,sizeof(exInfo.strName),TEXT("代长玉"));
// 	_sntprintf(exInfo.strAddress,sizeof(exInfo.strAddress),TEXT("福建省厦门市湖里区本虹网络信息科技有限公司"));
...
bool CAttemperEngineSink::OnDBPCSimpleLotteryResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	{
		sprintf(noti.szStr,"恭喜玩家[%s]转盘抽到大奖[%s]",pLotteryResult->szName,"888积分");
		m_pITCPNetworkEngine->SendDataBatch( MDM_GP_USER_SERVICE, SUB_GP_LOTTERY_NOTI, &noti, sizeof(noti),0);
	}
	if (pLotteryResult->dwIndex == 3)
	{
		sprintf(noti.szStr,"恭喜玩家[%s]转盘抽到大奖[%s]",pLotteryResult->szName,"999钻石");
		m_pITCPNetworkEngine->SendDataBatch( MDM_GP_USER_SERVICE, SUB_GP_LOTTERY_NOTI, &noti, sizeof(noti),0);
	}
	if (pLotteryResult->dwIndex == 6)
	{
		sprintf(noti.szStr,"恭喜玩家[%s]转盘抽到大奖[%s]",pLotteryResult->szName,"10房卡");
		m_pITCPNetworkEngine->SendDataBatch( MDM_GP_USER_SERVICE, SUB_GP_LOTTERY_NOTI, &noti, sizeof(noti),0);
	}
...
bool CAttemperEngineSink::OnDBGameShop(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	//这里测试用:DCY by 2017.3.11-----------------------------------------------------------
	tagShopInfo * pDBInfo=(tagShopInfo *)pData;
	TCHAR szOutPut[300] = {0};
	_sntprintf(szOutPut,sizeof(szOutPut),TEXT("商品信息:一共[%d]条,来自表THNativeWebDB.ShopInfo"),wDataSize/sizeof(tagShopInfo));
	//输出错误
...
// 	if (lResult == 1)
// 	{
// 		_sntprintf(szOutPut,sizeof(szOutPut),TEXT("兑换成功:来自表THNativeWebDB.ExchangeShop"));
// 	}
// 	else
// 	{
// 
// 		_sntprintf(szOutPut,sizeof(szOutPut),TEXT("兑换失败:来自表THNativeWebDB.ExchangeShop"));
// 	}
// 	//输出错误
// 	CTraceService::TraceString(szOutPut,TraceLevel_Exception);
	//这里测试用:DCY by 2017.3.11-----------------------------------------------------------
