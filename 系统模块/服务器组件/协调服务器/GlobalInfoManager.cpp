...
//////////////////////////////////////////////////////////////////////////////////
CGlobalRoomItem::CGlobalRoomItem()
{
	Reset();							

	m_pNextRoomItemPtr = NULL;
}
...
CGlobalInfoManager::CGlobalInfoManager()
{
	...
	m_MapUserRoom.InitHashTable(PRIME_PLATFORM_USER);
...
CGlobalInfoManager::~CGlobalInfoManager()
{
	...
	CGlobalRoomItem *pGlobalRoomItem = NULL;
	while (m_pGlobalRoomItem!=NULL)
	{
		pGlobalRoomItem = m_pGlobalRoomItem;
		m_pGlobalRoomItem = m_pGlobalRoomItem->m_pNextRoomItemPtr;
		SafeDelete(pGlobalRoomItem);
	}
...
DWORD CGlobalInfoManager::GetRoomItemCount(DWORD dwUserID)
{
	CWHDataLocker locker(m_CriticalSection);

	DWORD dwCnt = 0;
	CGlobalRoomItemArray * pGlobalRoomArray=NULL;
	m_MapUserRoom.Lookup(dwUserID,pGlobalRoomArray);
	if(NULL != pGlobalRoomArray)
	{
		dwCnt = pGlobalRoomArray->GetCount();
	}
	return dwCnt;
}
...
//删除房间
bool CGlobalInfoManager::DeleteRoomItem(DWORD dwUserID,DWORD dwRoomID)
{
	CWHDataLocker locker(m_CriticalSection);
	//寻找房间
	CGlobalRoomItemArray *pGlobalRoomItem = NULL;
	if(m_MapUserRoom.Lookup(dwUserID,pGlobalRoomItem) == FALSE)
	{
		ASSERT(FALSE);
		return false;
	}
	//变量定义
	for( int i = 0 ;i < pGlobalRoomItem->GetCount();i++)
	{
		CGlobalRoomItem *pGlobalItem = pGlobalRoomItem->GetAt(i);
		if(pGlobalItem == NULL) continue;
		
		if(pGlobalItem->m_dwRoomID == dwRoomID && pGlobalItem->m_dwCreatorID == dwUserID)
		{
			pGlobalRoomItem->RemoveAt(i);
			FreeGlobalRoomItem(pGlobalItem);
		}
	}

	if(pGlobalRoomItem->GetCount() == 0)
	{
		m_MapUserRoom.RemoveKey(dwUserID);
		SafeDelete(pGlobalRoomItem);
	}
...
bool CGlobalInfoManager::DeleteServerItem(WORD wServerID)
{
	...
	while (Position!=NULL)
	{
		...
		//释放玩家房间
		CGlobalRoomItemArray *pGlobalRoomItemArray = NULL;
		if(m_MapUserRoom.Lookup(pGlobalUserItem->GetUserID(),pGlobalRoomItemArray)==TRUE)
		{
			for(int i = 0; i < pGlobalRoomItemArray->GetCount();)
			{
				CGlobalRoomItem *pGlobalRoomItem = pGlobalRoomItemArray->GetAt(i);
				if(pGlobalRoomItem && pGlobalRoomItem->m_wServerID == wServerID)
				{
					pGlobalRoomItemArray->RemoveAt(i);
					continue;
				}
				i++;
			}
...
//激活房间
bool CGlobalInfoManager::ActiveRoomItem(DWORD dwUserID,tagGlobalRoomInfo &roomInfo)
{
	CWHDataLocker locker(m_CriticalSection);
	//寻找房间
	CGlobalRoomItemArray *pGlobalRoomItemArray = NULL;
	if(m_MapUserRoom.Lookup(dwUserID,pGlobalRoomItemArray) == TRUE)
	{
		for(int i = 0;i < pGlobalRoomItemArray->GetCount();i++)
		{
			CGlobalRoomItem *pRoomItem = pGlobalRoomItemArray->GetAt(i);
			if(pRoomItem->m_dwCreatorID == dwUserID && pRoomItem->m_dwRoomID == roomInfo.dwRoomID)
			{
				ASSERT(FALSE);
				return false;
			}
	...
	{
		pGlobalRoomItemArray = new CGlobalRoomItemArray;
		m_MapUserRoom[dwUserID]=pGlobalRoomItemArray;
	}
	CGlobalRoomItem *pRoomItem = CreateGlobalRoomItem();
	if(pRoomItem == NULL)
	{
		ASSERT(FALSE);
		return false;
	}
	pRoomItem->m_dwCreatorID = dwUserID;
	pRoomItem->m_wServerID = roomInfo.wServerID;
	pRoomItem->m_dwRoomID = roomInfo.dwRoomID;
	pRoomItem->m_cbJuShu = roomInfo.cbJuShu;
	pRoomItem->m_cbPlayer = roomInfo.cbPlayer;
	pRoomItem->m_cbSitPlayerCnt = roomInfo.cbSitPlayerCnt;
	pRoomItem->m_cbRoomState = roomInfo.cbRoomState;
	//设置房间
	pGlobalRoomItemArray->Add(pRoomItem);
	
	return true;
}

//修改房间信息
bool CGlobalInfoManager::UpdateRoomItem(DWORD dwUserID,tagGlobalRoomInfo &roomInfo)
{
	CWHDataLocker locker(m_CriticalSection);
...
CGlobalRoomItem * CGlobalInfoManager::EnumRoomItem(DWORD dwUserID,WORD wRoomIdx)
{
	CWHDataLocker locker(m_CriticalSection);

	//寻找用户
	CGlobalRoomItem *pRoomItem = NULL;
	CGlobalRoomItemArray *pRoomListArray = NULL;
	if(m_MapUserRoom.Lookup(dwUserID,pRoomListArray)==TRUE)
	{
		if(wRoomIdx < pRoomListArray->GetCount())
			pRoomItem = pRoomListArray->GetAt(wRoomIdx);
	}
...
CGlobalRoomItem * CGlobalInfoManager::CreateGlobalRoomItem()
{
	//使用存储
	if(m_pGlobalRoomItem!=NULL)
	{
		CGlobalRoomItem *pGlobalRoomItem = m_pGlobalRoomItem;
		m_pGlobalRoomItem = m_pGlobalRoomItem->m_pNextRoomItemPtr;
		pGlobalRoomItem->m_pNextRoomItemPtr = NULL;
	...
	try
	{
		CGlobalRoomItem * pGlobalRoomItem = new CGlobalRoomItem;
		return pGlobalRoomItem;
	}
	catch (...){}
...
//释放房间
bool CGlobalInfoManager::FreeGlobalRoomItem(CGlobalRoomItem * pGlobalRoomItem)
{
	//校验参数
	ASSERT(pGlobalRoomItem!=NULL);
	if(pGlobalRoomItem==NULL) return false;

	pGlobalRoomItem->Reset();

	pGlobalRoomItem->m_pNextRoomItemPtr = m_pGlobalRoomItem;
	m_pGlobalRoomItem = pGlobalRoomItem;
