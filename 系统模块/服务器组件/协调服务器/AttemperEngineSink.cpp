...
bool CAttemperEngineSink::OnEventTCPNetworkRead(TCP_Command Command, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case MDM_CS_ROOM_SERVICE:	//房间服务
		{
			return OnTCPNetworkMainRoomService(Command.wSubCmdID,pData,wDataSize,dwSocketID);
		}
	}
	...
	case SUB_CS_C_REGISTER_MATCH:	//注册比赛
		{
			...
			GameMatch.lInitScore = pRegisterMatch->lInitScore;
...
bool CAttemperEngineSink::OnTCPNetworkMainRoomService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	switch(wSubCmdID)
	{
	case SUB_CS_C_QUERYROOMLIST:
		{
			//参数校验
			ASSERT(sizeof(CMD_CS_C_QueryRoomList)==wDataSize);
			if(sizeof(CMD_CS_C_QueryRoomList)!=wDataSize) return false;

			//提取数据
			CMD_CS_C_QueryRoomList *pQueryData = (CMD_CS_C_QueryRoomList*)pData;

			QueryRoomList(dwSocketID,pQueryData->dwUserID,pQueryData->dwSocketID);
			return true;
		}
	case SUB_CS_C_ADDROOM:
		{
			//参数校验
			ASSERT(sizeof(CMD_CS_C_AddRoom)==wDataSize);
			if(sizeof(CMD_CS_C_AddRoom)!=wDataSize) return false;

			//提取数据
			CMD_CS_C_AddRoom *pAddRoom = (CMD_CS_C_AddRoom*)pData;
			tagGlobalRoomInfo roomData;
			roomData.dwCreatorID = pAddRoom->roomInfo.dwCreatorID;
			roomData.wServerID = pAddRoom->roomInfo.wServerID;
			roomData.cbJuShu = pAddRoom->roomInfo.cbJuShu;
			roomData.cbPlayer = pAddRoom->roomInfo.cbPlayer;
			roomData.cbRoomState = pAddRoom->roomInfo.cbRoomState;
			roomData.cbSitPlayerCnt = pAddRoom->roomInfo.cbSitPlayerCnt;
			roomData.dwRoomID = pAddRoom->roomInfo.dwRoomID;
			m_GlobalInfoManager.ActiveRoomItem(pAddRoom->dwUserID,roomData);

			return true;
		}
	case SUB_CS_C_DELETEROOM:
		{
			ASSERT(sizeof(CMD_CS_C_DeleteRoom)==wDataSize);
			if(sizeof(CMD_CS_C_DeleteRoom)!=wDataSize) return false;

			//提取数据
			CMD_CS_C_DeleteRoom *pNetData = (CMD_CS_C_DeleteRoom *)pData;

			m_GlobalInfoManager.DeleteRoomItem(pNetData->dwUserID,pNetData->dwRoomID);

			return true;
		}
	case SUB_CS_C_MODIFYROOM:
		{
			ASSERT(sizeof(CMD_CS_C_ModifyRoom)==wDataSize);
			if(sizeof(CMD_CS_C_ModifyRoom)!=wDataSize) return false;

			//提取数据
			CMD_CS_C_ModifyRoom *pNetData = (CMD_CS_C_ModifyRoom*)pData;
			tagGlobalRoomInfo roomData;
			roomData.dwCreatorID = pNetData->roomInfo.dwCreatorID;
			roomData.wServerID = pNetData->roomInfo.wServerID;
			roomData.cbJuShu = pNetData->roomInfo.cbJuShu;
			roomData.cbPlayer = pNetData->roomInfo.cbPlayer;
			roomData.cbRoomState = pNetData->roomInfo.cbRoomState;
			roomData.cbSitPlayerCnt = pNetData->roomInfo.cbSitPlayerCnt;
			roomData.dwRoomID = pNetData->roomInfo.dwRoomID;
			m_GlobalInfoManager.UpdateRoomItem(pNetData->dwUserID,roomData);
...
//查询房间列表
bool CAttemperEngineSink::QueryRoomList(DWORD dwSocketID,DWORD dwUserID,DWORD dwClientSocketID)
{
	//变量定义
	WORD wPacketSize=0L;
	WORD wRoomIdx = 0;
	CMD_CS_S_QueryRoomList_Result queryResult;
	queryResult.dwClientSocketID = dwClientSocketID;

	//收集数据
	do
	{
		//获取对象
		tagRoomListItem roomItem;
		CGlobalRoomItem * pGlobalRoomItem=m_GlobalInfoManager.EnumRoomItem(dwUserID,wRoomIdx);
		if (pGlobalRoomItem!=NULL)
		{
			roomItem.cbJuShu = pGlobalRoomItem->m_cbJuShu;
			roomItem.cbPlayer = pGlobalRoomItem->m_cbPlayer;
			roomItem.cbSitPlayerCnt = pGlobalRoomItem->m_cbSitPlayerCnt; 
			roomItem.dwRoomID = pGlobalRoomItem->m_dwRoomID;
			roomItem.cbRoomState = pGlobalRoomItem->m_cbRoomState;
			queryResult.roomList.push_back(roomItem);
		}
		else
		{
			break;
	...
	datastream sendData;
	queryResult.StreamValue(sendData,true);
	wPacketSize = sendData.size();
	//发送数据
	if (wPacketSize>0) m_pITCPNetworkEngine->SendData(dwSocketID,MDM_CS_ROOM_SERVICE,SUB_CS_S_QUERYROOMLIST,sendData.data(),sendData.size());
