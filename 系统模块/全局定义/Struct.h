...
//房间信息
struct tagGlobalRoomInfo
{
	DWORD						dwRoomID;							//房间ID
	WORD						wServerID;							//服务ID
	DWORD						dwCreatorID;						//房间创建者ID
	BYTE						cbRoomState;							//房间状态
	BYTE						cbJuShu;							//局数模式
	BYTE						cbPlayer;							//玩家人数
	BYTE						cbSitPlayerCnt;						//已坐下玩家数量
};

struct CMD_CS_C_AddRoom
{
	DWORD							dwUserID;						//用户ID
	tagGlobalRoomInfo				roomInfo;						//房间信息
};

struct CMD_CS_C_ModifyRoom
{
	DWORD							dwUserID;						//用户ID
	tagGlobalRoomInfo				roomInfo;						//房间信息
};

struct CMD_CS_C_DeleteRoom
{
	DWORD							dwUserID;						//用户ID
	DWORD							dwRoomID;							//房间信息
};

struct tagRoomListItem
{
	DWORD							dwRoomID;							//房间号
	BYTE							cbJuShu;							//局数模式
	BYTE							cbPlayer;							//玩家人数
	...
	{
		Stream_VALUE(dwRoomID);
		Stream_VALUE(cbJuShu);
		Stream_VALUE(cbPlayer);
		Stream_VALUE(cbSitPlayerCnt);
		Stream_VALUE(cbRoomState);
	}
};

struct CMD_GP_QueryRoomList_Result
{	
	std::vector<tagRoomListItem> roomList;
	void StreamValue(datastream& kData,bool bSend)
	{
		StructVecotrMember(tagRoomListItem,roomList);
	}
};

struct CMD_CS_S_QueryRoomList_Result
{
	DWORD dwClientSocketID;
	std::vector<tagRoomListItem> roomList;
	void StreamValue(datastream& kData,bool bSend)
	{
		Stream_VALUE(dwClientSocketID);
		StructVecotrMember(tagRoomListItem,roomList);
	}
};
//////////////////////////////////////////////////////////////////////////////////
...
struct tagShopInfo
{
	...
	SCORE							llPrice;							//商品价格
	DWORD							dwCount;							//商品数量
	TCHAR							strPath[0xFF];						//下载路径
	BYTE							cbPriceType;						//价格类型
	TCHAR							szName[50];							//名称
	WORD							wGiveGold;							//赠送金币
	WORD							wGiveScore;							//赠送积分
};
//////////////////////////////////////////////////////////////////////////////////
